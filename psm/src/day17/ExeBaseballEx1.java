...
public class ExeBaseballEx1 {
	...
	public static void main(String[] args) {
		/* 컬렉션 프레임 워크를 이용하여 숫자 야구 게임을 구현해보세요.*/
	...
	public static void createRandomLIst(List<Integer> list, int min, int max,int count ) {
		...
		if(count > max - min + 1) {
			throw new RuntimeException("범위가 배열의 크가보다 작아서 만들수없습니다.");
		}
		//중복되지 않은 숫자를 생성하기 위한 set을 선언 및생성
	...
	/* 기능 : 정수 리스트가 주어지면 해당 정수 리스트에 min~max 사이의 값을 스태너를 통해
	 * 		 count개 입력받아 제대로 입력했는지를 알려주는 메소드
	 * 		 0 : 제대로 입력, 1 : 범위 잘못, -1 : 중복 
	 * 매개변수 : 리스트, 범위, 갯수, 스캐터 =>  
	 * 			List<Integer> list, int min, int max,int count,
	 *			Scanner scan
	 * 리턴타입 : 0또느 1또는 -1 => 정수 => int
	 * 메소드명 : inputList 
	 * */
