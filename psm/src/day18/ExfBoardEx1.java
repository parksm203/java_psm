...
public class ExfBoardEx1 {
	...
	do {
		...
		while(true) {
			...
			if(ch =='1') {
				...
				System.out.println("게시글이  수정이 완료 되었습니다.");
				System.out.println("--------------------------");
				}
				//없으면 없다고 안내문구 출력
				else {
	...
	/*	
		...
		case 1:
			...
			contents = scan1.nextLine();
			System.out.println("작성자");
			writer = scan1.nextLine();
			System.out.println("작성일");
			date = scan1.nextLine();
			boardList[count]
					= new Board(count+1, title, contents, writer,date);
				count++;
			break;
		}
		...
		case 3:
			...
			if(num <= count) {
				if(boardList [num-1] == null) {
					System.out.println("삭제된 게시글은 수정할수 없음.");
				}else {
					scan1.nextLine();
					System.out.println("게시글 제목");
					title = scan1.nextLine();
