...
public class MethodArrayContainsEx1 {
	...
	/* * 기능 : 배열과 정수가 주어지면 정수가 배열에 있으면 true, 없으면 false를 알려주는 메소드
	...
	/* 기능 : 배열 arrdhk 정수 num가 주어지면 배열에서 0번지부터 n중에 num가 있는지
	 *       없는지 알려주는 메소드
	 * 매개변수 : 배열과 정수, 확인할 갯수 => int []arr, int num, int n 
	 * 리턴타입 : 있다/없다 => ture/false
	 * 메소드명 :containArray2
	 * 
	 * */
	public static boolean containsArray2(int []arr, int num, int n) {
		...
		//아니면 원래 검사 갯수로 변경
		n = arr.length< n ? arr.length : n; //이부분이 있어야 안정적으로 돌아감.
		for(int i = 0; i<n; i++) {
			if(arr[i] == num) {
				return true;
			}
