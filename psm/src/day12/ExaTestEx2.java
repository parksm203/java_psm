...
public class ExaTestEx2 {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i<list.length; i++){
			...
			* - 참조 변수는 null로 초기화
			* - 도형이 없다는 건 도형이 생성 되어 저장된게 아니기 때문에 초기값인 경우이다*/
			if(list[i] != null) {
				list[i].print();
			
			}
		}
		/* 저장된 도형이 타원이면 testE()수출하고, 사각형이면 teseR()을 호출하세요.
		 * 반복문을 이용*/
		for(ExaShape tmp : list) {
			if( tmp != null) {
				//도형이 타원이면 testE()수출하고, 사각형이면 teseR()을 호출
				if(tmp instanceof ExaEllipse) {
					//도형 tmp를 타원으로 형변환 시킨후 testE()을 호출
					((ExaEllipse)tmp).testE();		   // <---  1. 첫번째 방법
					/*ExaEllipse e =(ExaEllipse)tmp;      <---- 2. 두번째 방법
					e.testE(); */
				}
				// 도형이 사각형이면 teseR()을 호출
